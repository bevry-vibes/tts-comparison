...
```
**Restructure (her note: "the tables are all over the place, three tables for Indonesian"):** index.html is now the audition page — A · English matrix, A2 · full sample set, B · word render A/B, C · one merged Indonesian table, D · phoneme library, E · historical records; the settled one-time A/Bs (Kokoro artifact, breath, IPA input, Higgs deep-dive, sandwich sources) moved to `diagnostics.html`. **The full 484-file phonetic-samples category was re-classified with a multilingual descriptor matcher** (English + Spanish + French/PR- naming — Spanish files like "Oclusiva bilabial sorda" = p had been invisible to the English-only matcher) → 27 more takes for the thin phonemes (p t k g f l s z ʃ ʒ tʃ dʒ θ j ə). Classifier + downloader in `~/.cache/sumba-tts/classify_full_category.py` / `download_classified.py`.
